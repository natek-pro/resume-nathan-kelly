...
Deployment runs automatically on `main` only after **both** `validate` and `encrypted` succeed. Failed, cancelled, or skipped prerequisites prevent publishing; PRs and other branches never deploy. The deployment job downloads only the current run's named public PDF and web artifacts, verifies its SHA-256 against the validation job's output, commits those exact files into `natek-pro/natek.pro`, then polls the live HTTPS URL until both SHA-256 hashes match. It does not rebuild the PDF or download private artifacts. GitHub Pages publishes the website commit. A publication or verification failure fails the deployment job; the run summary links the verified PDF only on success.
